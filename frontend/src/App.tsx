...
import {
    Phone, CheckCircle2, XCircle, Loader2, User,
    Clock, RefreshCw,
    PhoneForwarded, X, Mic, Bot, Users, Calendar, Sparkles
} from 'lucide-react';
...
function App() {
    ...
    const handleCall = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                showToast('Call failed: ' + data.error);
    ...
    const agentStatusCfg: Record<AgentCallStatus, any> = {
        ...
        calling:       { cls: 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30', label: 'Dialing Agent...', icon: <Loader2 className="w-3 h-3 animate-spin" /> },
                    ...
                    <div className="w-full max-w-5xl flex flex-col items-center gap-12">
                        <div className="w-full max-w-2xl relative group mt-10">
                            <div className="spotlight-bar flex items-center p-2 relative z-20">
                                ...
                                <button
                                    onClick={handleCall}
                                    disabled={callStatus === 'in-progress' || callStatus === 'calling' || phoneNumber.length !== 14}
                                ...
                                >
                                    {callStatus === 'calling' ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Call Now'}
                                </button>
                            </div>
                        ...
                        <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="glass-panel p-6 flex flex-col gap-4">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <BentoItem label="Phone" value={liveData.phone} mono={true} />
                                </div>
                            </div>
                            ...
                            <div className="glass-panel p-6 flex flex-col gap-4 col-span-2">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <BentoItem label="Product Interest" value={liveData.product_interest} highlight={true} />
                                    <BentoItem label="Monthly Income" value={liveData.monthly_income} />
                                    <BentoItem label="Loan Amount" value={liveData.loan_amount_range} highlight={true} />
                                    <BentoItem label="Employment" value={liveData.employment_type} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="w-full glass-panel p-6">
                            ...
                            {history.length === 0 ? (
                                <p className="text-center py-10 text-zinc-600 italic text-sm">No calls yet. Complete a call to see it here.</p>
                            ) : (
                                        ...
                                        <tbody className="text-sm text-zinc-300 divide-y divide-white/5">
                                            {history.map(row => (
                                                <tr key={row.id} className="hover:bg-white/[0.02] transition-colors">
                                                    <td className="py-3 px-4 font-medium text-white">{row.full_name || '—'}</td>
                                                    <td className="py-3 px-4 font-mono text-zinc-400">{row.phone || '—'}</td>
                                                    <td className="py-3 px-4 text-purple-300">{row.product_interest || '—'}</td>
                                                    <td className="py-3 px-4 text-zinc-400">{row.city || '—'}</td>
                    ...
                    <div className="w-full max-w-5xl flex flex-col items-center gap-12 mt-10">
                            ...
                            <div className="glass-panel p-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="bento-card border-purple-500/20 bg-purple-500/5">
                                        <p className="text-xs text-purple-300 font-medium mb-1">Product Interest</p>
                                        <p className="text-lg font-bold text-white">{liveData.product_interest || 'Unknown'}</p>
                                    </div>
                                    <div className="bento-card border-indigo-500/20 bg-indigo-500/5">
                                        <p className="text-xs text-indigo-300 font-medium mb-1">Loan Amount</p>
                                        <p className="text-lg font-bold text-white">{liveData.loan_amount_range || 'Not Identified'}</p>
                                    </div>
                                    <div className="bento-card border-rose-500/20 bg-rose-500/5">
                                        <p className="text-xs text-rose-300 font-medium mb-1">Employment</p>
                                        <p className="text-lg font-bold text-white">{liveData.employment_type || 'None recorded'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="glass-panel p-6 flex flex-col">
                                ...
                                {callbacks.length === 0 ? (
                                    <p className="text-center py-10 text-zinc-600 italic text-sm">No callbacks scheduled yet.</p>
                                ) : (
                                    <div className="relative border-l-2 border-white/10 ml-3 space-y-6">
                                        {callbacks.map(cb => (
                                            <div key={cb.id} className="relative pl-6">
                                                ...
                                                <div className="bento-card p-4 hover:border-purple-500/30 transition-colors">
                                                    ...
                                                    <p className="mt-3 text-xs text-purple-300 font-semibold bg-purple-500/10 px-2 py-1.5 rounded-md inline-flex items-center gap-1.5"><Clock className="w-3 h-3"/> {cb.scheduledTime}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="glass-panel p-6 flex flex-col">
                                ...
                                {recordings.length === 0 ? (
                                    <p className="text-center py-10 text-zinc-600 italic text-sm">No recordings yet.</p>
                                ) : (
                                    <div className="space-y-4">
                                        {recordings.map(rec => (
                                            <div key={rec.id} className="bento-card">
                                                ...
                                                <audio controls src={`${BACKEND_URL}/api/recordings/${rec.id}/audio`} className="w-full h-10 rounded-full" />
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            <div className="flex-1 flex flex-col border-b md:border-b-0 md:border-r border-white/10">
                                ...
                                <div className="flex-1 overflow-y-auto p-6 space-y-4" ref={transcriptContainerRef}>
                                    {transcripts.length === 0 ? (
                                        <p className="text-center text-zinc-600 italic text-sm pt-10">Waiting for agent call...</p>
                                    ) : transcripts.map((t, i) => (
                                        <div key={i} className={`p-4 rounded-2xl max-w-[85%] ${t.role === 'assistant' ? 'bg-indigo-500/10 border border-indigo-500/20 ml-auto rounded-tr-sm' : 'bg-white/5 border border-white/10 rounded-tl-sm'}`}>
                                            <p className="text-xs font-semibold text-zinc-400 mb-1">{t.role === 'assistant' ? 'Finfinity AI' : 'Customer'}</p>
                            ...
                            <div className="flex-1 flex flex-col bg-black/20">
                                ...
                                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                                    {rmTranscripts.length === 0 ? (
                                        <p className="text-center text-zinc-600 italic text-sm pt-10">RM transcript will appear here...</p>
                                    ) : rmTranscripts.map((t, i) => (
                                        <div key={i} className={`p-4 rounded-2xl max-w-[85%] ${t.role === 'assistant' ? 'bg-purple-500/10 border border-purple-500/20 ml-auto rounded-tr-sm' : 'bg-white/5 border border-white/10 rounded-tl-sm'}`}>
                                            <p className="text-xs font-semibold text-zinc-400 mb-1">{t.role === 'assistant' ? 'Human RM' : 'Customer'}</p>
                                            <p className="text-sm text-zinc-200 leading-relaxed">{t.content}</p>
                                        </div>
                                    ))}
                                </div>
